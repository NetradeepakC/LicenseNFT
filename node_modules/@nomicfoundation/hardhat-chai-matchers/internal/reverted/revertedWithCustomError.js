...
function supportRevertedWithCustomError(Assertion, utils) {
    Assertion.addMethod("revertedWithCustomError", function (contract, expectedCustomErrorName) {
        ...
        const onSuccess = () => {
            this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it didn't revert`);
        };
        const onError = (error) => {
            ...
            if (decodedReturnData.kind === "Empty") {
                this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it reverted without a reason`);
            }
            else if (decodedReturnData.kind === "Error") {
                this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it reverted with reason '${decodedReturnData.reason}'`);
            }
            else if (decodedReturnData.kind === "Panic") {
                this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it reverted with panic code ${decodedReturnData.code.toHexString()} (${decodedReturnData.description})`);
            }
                ...
                if (decodedReturnData.id === expectedCustomError.id) {
                    ...
                    this.assert(true, null, `Expected transaction NOT to be reverted with custom error '${expectedCustomErrorName}', but it was`);
                }
                else {
                    ...
                    if (actualCustomError === undefined) {
                        this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it reverted with a different custom error`);
                    }
                    else {
                        this.assert(false, `Expected transaction to be reverted with custom error '${expectedCustomErrorName}', but it reverted with custom error '${actualCustomError.name}'`);
                    }
                }
            }
            else {
...
async function revertedWithCustomErrorWithArgs(context, Assertion, utils, expectedArgs) {
    ...
    const actualArgs = contractInterface.decodeErrorResult(errorFragment, returnData);
    ...
    for (const [i, actualArg] of Object.entries(actualArgs)) {
        const expectedArg = expectedArgs[i];
        if (typeof expectedArg === "function") {
            const errorPrefix = `The predicate for custom error argument #${parseInt(i, 10) + 1}`;
            try {
                context.assert(expectedArg(actualArg), `${errorPrefix} returned false`
                ...
                if (e instanceof chai_1.AssertionError) {
                    context.assert(false, `${errorPrefix} threw an AssertionError: ${e.message}`
                    // no need for a negated message, since we disallow mixing .not. with
                    // .withArgs
                    );
                }
