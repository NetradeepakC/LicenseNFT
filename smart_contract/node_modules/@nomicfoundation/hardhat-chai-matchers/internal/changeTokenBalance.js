...
function supportChangeTokenBalance(Assertion) {
    Assertion.addMethod("changeTokenBalance", function (token, account, balanceChange) {
        ...
        const derivedPromise = Promise.all([
            ...
            this.assert(actualChange.eq(ethers.BigNumber.from(balanceChange)), `Expected the balance of ${tokenDescription} tokens for "${address}" to change by ${balanceChange.toString()}, but it changed by ${actualChange.toString()}`, `Expected the balance of ${tokenDescription} tokens for "${address}" NOT to change by ${balanceChange.toString()}, but it did`, balanceChange, actualChange);
        });
    ...
    Assertion.addMethod("changeTokenBalances", function (token, accounts, balanceChanges) {
        ...
        const derivedPromise = Promise.all([
            ...
            this.assert(actualChanges.every((change, ind) => change.eq(ethers.BigNumber.from(balanceChanges[ind]))), `Expected the balances of ${tokenDescription} tokens for ${addresses} to change by ${balanceChanges}, respectively, but they changed by ${actualChanges}`, `Expected the balances of ${tokenDescription} tokens for ${addresses} NOT to change by ${balanceChanges}, respectively, but they did`, balanceChanges.map((balanceChange) => balanceChange.toString()), actualChanges.map((actualChange) => actualChange.toString()));
        });
        this.then = derivedPromise.then.bind(derivedPromise);
        this.catch = derivedPromise.catch.bind(derivedPromise);
        return this;
...
async function getBalanceChange(transaction, token, account) {
    ...
    (0, utils_1.ensure)(block.transactions.length === 1, Error, "Multiple transactions found in block");
