...
function supportChangeEtherBalances(Assertion) {
    Assertion.addMethod("changeEtherBalances", function (accounts, balanceChanges, options) {
        ...
        const derivedPromise = Promise.all([
            ...
            this.assert(actualChanges.every((change, ind) => change.eq(BigNumber.from(balanceChanges[ind]))), () => {
                const lines = [];
                ...
                return lines.join("\n");
            }, balanceChanges.map((balanceChange, i) => `Ether balance of ${accountAddresses[i]} changed by ${balanceChange.toString()} wei`), actualChanges.map((actualChange, i) => `Ether balance of ${accountAddresses[i]} changed by ${actualChange.toString()} wei`));
        });
